...
$.widget( "ui.hbp_slider", $.ui.slider, 
	{
		...
		_create: function()
		{
            ...
            if( $("style:contains('slider_text')").length < 1 )
                $("<style type='text/css'>.slider_text { font-family: 'Verdana'; color: #FFFFFF; font-size: 10; position:absolute } </style>").appendTo("head");
            ...
            if( $("style:contains('" + callout_class + "')").length < 1 )
                $("<style type='text/css'>." + callout_class + " { width: " + s + "px; margin-left: -" + Math.floor(s/2) + "px; position: absolute; top: -33px; text-align: center;  -moz-border-radius: 6px; border-radius: 6px; background:#5A5A4B; color:#FFFFFF; left:0px;  } </style>").appendTo("head");
        ...
        _setOption: function( key, value )
        {
            ...
            switch ( key ) {
                case "min":
                    this.minDisplay.text( this.prefix + $.format.number( value, this.options.valueFormat ) + this.suffix );
                    break;
                case "max":
                    this.maxDisplay.text( this.prefix + $.format.number( value, this.options.valueFormat ) + this.suffix );
                    break;
            }
        },
